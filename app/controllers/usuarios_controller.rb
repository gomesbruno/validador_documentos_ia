class UsuariosController < ApplicationController
  ...
  before_action :set_usuario, only: [:show, :edit, :update, :destroy, :reativar]
  ...
  def destroy
    resp = UsuarioService.desativar_usuario(@usuario)
    redirect_to usuarios_url, notice: resp[:notice], alert: resp[:alert]
  end

  def reativar
    resp = UsuarioService.reativar_usuario(@usuario)
    redirect_to usuarios_url, notice: resp[:notice], alert: resp[:alert]
  end
  ...
  def autocomplete_usuario_identificacao_login
    @results = Usuario.includes(identificacao_login: :dados_identificacao)
                      .por_iduff_ou_nome(params[:term])
                      .limit(10)
    render json: @results.map { |u| { id: u.id, label: autocomplete_label(u), value: autocomplete_label(u) } }
  end
  ...
  def perfil_params
    params.fetch(:usuario, {}).fetch(:opcoes_perfil, [])
  end
  ...
  def colunas_usuarios
    ['Nome', 'CPF', 'Perfis', 'Situação', 'Ações']
  end


  def autocomplete_label(usuario)
    identificacao = usuario.identificacao_login
    cpf = identificacao&.dados_identificacao&.cpf
    [identificacao&.nome, cpf, identificacao&.iduff].compact.join(' - ')
  end
