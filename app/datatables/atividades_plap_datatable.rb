class AtividadesPlapDatatable < ApplicationDatatable
  ...
  def data_inicio
    params[:data_inicio].presence&.to_datetime || AcompanhamentoStatus.first.created_at.to_datetime
  end
    ...
    dados.map do |acompanhamentos|
      {
        ...
        projetos_analisados: acompanhamentos[:acompanhamento_status],
        apostilamentos_realizados: acompanhamentos[:acompanhamentos_status_apostilamentos],
        termos_aditivos_realizados: acompanhamentos[:acompanhamentos_status_termos_aditivos],
        pagamentos_realizados: acompanhamentos[:acompanhamentos_status_pagamentos],
      }
    end
  end
  ...
  def todas_datas(data_inicial,data_final)
    monts_years_between(data_inicial.to_date, data_final.to_date).map do |mes_ano|
      {data: mes_ano.strftime("%m/%Y"),
       acompanhamento_status: 0,
       acompanhamentos_status_apostilamentos: 0,
       acompanhamentos_status_termos_aditivos: 0,
       acompanhamentos_status_pagamentos: 0 }
    end
  end
  ...
  def transforma_datas(datas_agrupadas)
    ...
    data_transformada.each do |key,value|
      ...
      data_transformada[key][:acompanhamento_status] ||= 0
      data_transformada[key][:acompanhamentos_status_apostilamentos] ||= 0
      data_transformada[key][:acompanhamentos_status_termos_aditivos] ||= 0
      data_transformada[key][:acompanhamentos_status_pagamentos] ||= 0
    end
  end
  ...
  def monta_dados(data_inicial, data_final)
    ...
    pagamentos = pagamentos_realizados(data_inicial, data_final)
    ...
    { projetos_analisados:projetos, apostilamentos_realizados: apostilamentos,
          termos_aditivos_realizados: termos, pagamentos_realizados:pagamentos }
  end
  ...
  def projetos_analisado(data_inicial, data_final)
    AcompanhamentoStatus.
      ...
      select("count(acompanhamento_status.id) as quantidade, DATE_FORMAT(acompanhamento_status.created_at,'%m/%Y') as data").
      group("data")
  end
  ...
  def termos_aditivos_realizados(data_inicial, data_final)
    AcompanhamentoStatusTermoAditivo.
      por_etapa(:finalizado).
      entre_datas_criacao(data_inicial,data_final).
      select("count(acompanhamentos_status_termos_aditivos.id) as quantidade, DATE_FORMAT(acompanhamentos_status_termos_aditivos.created_at,'%m/%Y') as data").
      group("data")
  end

  def pagamentos_realizados(data_inicial, data_final)
    AcompanhamentoStatusPagamento.
      por_etapa(:finalizado).
      entre_datas_criacao(data_inicial,data_final).
      select("count(acompanhamentos_status_pagamentos.id) as quantidade, DATE_FORMAT(acompanhamentos_status_pagamentos.created_at,'%m/%Y') as data").
      group("data")
  end
